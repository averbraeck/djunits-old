...
public class ScalarOperationsTest
{
            ...
            NoSuchFieldException, SecurityException, IllegalArgumentException, ClassNotFoundException
    {
        ...
        for (int i = 0; i < CLASSNAMES.ABS_LIST.size(); i++)
        {
            ...
            try
            {
                scalarClassAbs = Class.forName("org.djunits4.value.v" + type + ".scalar." + scalarClassNameAbs);
            }
            catch (ClassNotFoundException exception)
            {
                fail("Class Rel not found for " + upperType + "Scalar class " + "org.djunits4.value.v" + type + ".scalar."
                        + scalarClassNameAbs);
            }
            try
            {
                scalarClassRel = Class.forName("org.djunits4.value.v" + type + ".scalar." + scalarClassNameRel);
            }
            catch (ClassNotFoundException exception)
            {
                fail("Class Rel not found for " + upperType + "Scalar class " + "org.djunits4.value.v" + type + ".scalar."
        ...
        for (String scalarName : CLASSNAMES.REL_LIST)
        {
            ...
            try
            {
                scalarClassRel = Class.forName("org.djunits4.value.v" + type + ".scalar." + scalarClassName);
            }
            catch (ClassNotFoundException exception)
            {
                fail("Class Rel not found for " + upperType + "DoubleScalar class " + "org.djunits4.value.v" + type + ".scalar."
                        + scalarClassName);
            }
            testMethods(scalarClassRel, false, doubleType);
        }
            ...
            IllegalAccessException, InvocationTargetException, NoSuchFieldException
    {
        ...
        try
        {
            relativeOrAbsoluteClass = Class.forName("org.djunits4.value." + (abs ? "Absolute" : "Relative"));
        }
        catch (ClassNotFoundException exception)
        {
            fail("Could not find org.djunits4.value.Relative class");
        }
