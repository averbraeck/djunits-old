 ...
 * <i>FloatScalar.Rel&lt;ElectricalPotentialUnit&gt; value = new FloatScalar.Rel&lt;ElectricalPotentialUnit&gt;(100.0, ElectricalPotentialUnit.SI);</i><br>
 * we can now write <br>
...
public class FloatElectricalPotential extends FloatScalar.Rel<ElectricalPotentialUnit> implements Relative
{
    ...
    {
        return new FloatElectricalPotential(zero.getInUnit() * (1 - ratio) + one.getInUnit(zero.getUnit()) * ratio, zero.getUnit());
    }
     ...
     * @return a Scalar at the ratio between
     */
    public static FloatElectricalPotential interpolate(final FloatElectricalPotential zero, final FloatElectricalPotential one, final double ratio)
    {
    ...
    {
        return getUnit().equals(v.getUnit()) ? new FloatElectricalPotential(getInUnit() + v.getInUnit(), getUnit()) : new FloatElectricalPotential(this.si
            + v.si, ElectricalPotentialUnit.SI);
    }
