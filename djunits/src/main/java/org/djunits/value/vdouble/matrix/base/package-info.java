 ...
 * from 1 to m, and j ranges from 1 to n. This is known as an m x n matrix, with (m, n) as its size. So when a double array
 * d[][] is passed to fill a matrix, the most logical is that the first index indicates the row, and the second index indicates
 * the column, so d[row][column]. In Java this means that the matrix needs to be initialized with
 * <code><b>new</b> d[rows]</code> and each row needs to be initialized with <code><b>new</b> d[r][columns]</code>.
 * <p>
